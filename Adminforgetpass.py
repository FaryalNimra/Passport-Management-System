...
"")
    ...
    def change(self):
        a_User_name = (self.lineEdit_4.text())
        a_new_password = (self.lineEdit_2.text())
        a_new_con_Passwrd = (self.lineEdit_3.text())
        msg = QMessageBox()
        if a_new_con_Passwrd == a_new_password:
            try:
                with cx_Oracle.connect('system/Oracle_1') as co:
                    cur = co.cursor()
                    cur.callproc("UPDATE_ADMIN_T1", [
                        a_User_name,
                        a_new_password,
                        a_new_con_Passwrd
                    ])
