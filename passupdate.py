...
"")
        ...
        self.GenerateBill.setGeometry(QtCore.QRect(250, 430, 251, 51))
        ...
        self.GenerateBill.setStyleSheet("QPushButton{\n"
                                        ...
                                        "\n"
                                        "}")
        self.GenerateBill.clicked.connect(lambda: self.pass_update())
        self.GenerateBill.setObjectName("Passport update")
        self.ExitButton = QtWidgets.QPushButton(self.frame)
        self.ExitButton.setGeometry(QtCore.QRect(550, 430, 251, 51))
...
"")
    ...
    def retranslateUi(self, Dialog):
        ...
        self.GenerateBill.setText(_translate("Dialog", "Insert /Update "))
    ...
    def pass_update(self):
        ...
        if self.F_Name.text() == "" or self.MeterNo.text() == "" or self.Passwrd.text() == "":
            ...
            w = msg.exec_()
                ...
                try:
                    p_cnic_no = self.F_Name.text()
                    p_new_First_name= self.M_Name.text()
                    p_new_Last_name= self.L_Name.text()
                    p_new_dob = self.MeterNo.text()

                    p_new_address = self.RefNo.text()
                    p_new_nationality = self.Passwrd.text()
                    p_new_contact_info = self.H_No.text()
                    p_new_Disability = self.Street_No.text()
                    p_new_Province = self.U_Division.text()
                    p_new_City= self.U_City.text()
                    p_new_Gender = self.Gender.text()
                    ...
                    with cx_Oracle.connect('system/Oracle_1') as co:
                        ...
                        cur.callproc("UPDATE_PERSON_T11", [
                            p_cnic_no,
                            p_new_First_name,
                            p_new_Last_name,
                            p_new_dob,
